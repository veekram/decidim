...
module Decidim
  module Proposals
    describe ProposalEndorsement do
      ...
      let!(:user_group) { create(:user_group, verified_at: Time.zone.now, organization: organization, users: [author]) }
      ...
      context "when retrieving for_listing" do
        ...
        let!(:other_user_group) { create(:user_group, verified_at: Time.zone.now, organization: author.organization, users: [author]) }
