...
describe "decidim_initiatives:check_validating", type: :task do
  let(:threshold) { Time.zone.now - Decidim::Initiatives.max_time_in_validating_state }
  ...
  context "when initiatives without changes" do
    let(:initiative) { create(:initiative, :validating, updated_at: Time.zone.now - 1.year) }
