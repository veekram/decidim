...
describe "decidim_initiatives:notify_progress", type: :task do
  ...
  context "when initiative ready for second notification" do
    let(:initiative) do
      initiative = create(:initiative, first_progress_notification_at: Time.zone.now)
  ...
  context "when initiative with both notifications sent" do
    let(:initiative) do
      create(:initiative,
             first_progress_notification_at: Time.zone.now,
             second_progress_notification_at: Time.zone.now)
    end
